...
BPTree::BPTree(const Rcpp::S4 & treeSE) {
    const Rcpp::S4 & rowTree = treeSE.slot("RowTree");
    structure = std::vector<bool>();
    structure.reserve(500000);  // a fair sized tree... avoid reallocs, and its not _that_ much waste if this is wrong
}
...
void BPTree::rowTree_to_bp(const Rcpp::List & rowTree) {
    ...
    std::stack<uint32_t> nodes = std::stack<uint32_t>();  // phylo tips are always numbered from 1 to number of tips;
    std::vector<uint32_t> levels = std::vector<uint32_t>(ntips, 0); // Use this to keep track of how many descendants the current branch's node have
    ...
    for (int i = 0; i < edge.nrow(); i++){
        ...
        if(nodes.size() == 0 || nodes.top() != currentNode ) { // Either were at the root, or entering a new node
            if(nodes.top() < currentNode) { // We've exhausted the node and moved backwards in the tree
        ...
        if(nextNode <= ntips) { // Node is a tip
            count += 2;
            structure.push_back(true);
